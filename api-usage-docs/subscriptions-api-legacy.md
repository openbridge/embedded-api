# Subscriptions API (Legacy v1)

> **This is the legacy v1 API.** New integrations should use the [Subscriptions API (v2)](./subscriptions-api.md) instead. V1 responses include a `Deprecation` response header. The unversioned paths shown below (`/sub`, `/spm`, `/storages`, `/product`) are legacy aliases for `/v1/...` — you can also call them with an explicit `/v1/` prefix (e.g. `/v1/sub`).
