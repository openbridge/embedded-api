...
If you want to turn off a pipeline either permentantly or temporarily you need change the status of the record on file.  This is done with a patch request. Through the [Subscription API](../README.md#subscription-api) and is described in our API documentation.
...
If you want to update an attribute of a pipeline subscription, you can use a PATCH request using the [Subscription API](../README.md#subscription-api) as described in our API documentation.  It is important to note that when sending a PATCH request to the subscription API update that you always include the `status` field as part of the PATCH request.  Even if the `status` isn't changing.
